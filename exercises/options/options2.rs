...
mod tests {
    ...
    fn layered_option() {
        ...
        while let Some(integer) = optional_integers.pop() { // This .pop() returns an option, so you get an Option<Option<T>>
            if let Some(i) = integer { // This if let Some prevents None in block
                assert_eq!(i, cursor);
                cursor -= 1;
            }
        }
